...
var EventServiceTest = function () { 
	...
	return {
		init : function() {
			alert("EventServiceTest: init ");
			console.log("EventServiceTest: init ");
			EventStorageDao.init();
			EventServiceTest.sync(function(){
				console.log("nothing to update");
			})
		},
		clear : function(){
			console.log("EventServiceTest: clear ");
			EventStorageDao.clear();
		},
		createEvent : function (callback, jsonDataToBePosted) {
			console.log("EventServiceTest: createEvent ");
			...
			AjaxEventHelper.createPOSTRequestAjax( url, 
												   function( eventJson ){
														...
														EventStorageDao.addOrUpdateEvent(event);			
														callback(eventJson);
													},
													jsonDataToBePosted); 
		},
		deleteEventById : function(eventId, callback) {
			console.log("EventServiceTest: deleteEventById: " + eventId);
			var url = AjaxEventHelper.getRootURL() + 'events/'+ eventId;
			AjaxEventHelper.createDELETERequestAjax(url,
													function ( data ){
														EventStorageDao.removeEvent(eventId);
														callback(data)
													})
		},
		getLastSync: function(callback) {
			EventStorageDao.getLastSyncDate(callback);
	    },
	        		...
	        		function(lastSync){
			            getChanges(syncURL, lastSync, function (data) {
			        		...
			        		if(data.length > 0) {
			        			...
			        			EventStorageDao.syncEventList(events, callback);
			        		} else {
	    ...
	    findEventsByuserId : function (userId, callback){
	    	console.log("EventServiceTest:findEventsById: " + userId);
			EventStorageDao.findEventsByuserId(userId, function(_eventJS) {
														callback(_eventJS);
													 });
	    },
		findEventById : function (eventId, callback){
			console.log("EventServiceTest:findEventById: " + eventId);
			EventStorageDao.findEventById(eventId, function(_eventJS) {
														callback(_eventJS);
													 });
			
		}, 
		findEventsforToday : function (date, callback){
			console.log("EventServiceTest:findEventsforToday: " + date);
			EventStorageDao.findEventforToday(date, 
					function(list) {
						var map = new Object();
	    ...
	    findAllEventsGroupedByCity : function(callback) {
	    	console.log("EventServiceTest:findAllEventsGroupedByCity");
			EventStorageDao.findAll( new Date(),  
							    function(list){
									var map = new Object();
									var _eventJson, _eventJS, city ;
									for(i=0;i < list.length; i++){
		...
		findEventsByCity : function(cityId, callback){
			console.log("EventServiceTest:findEventsByCity: " + cityId);
			EventStorageDao.findEventsByCity( 
					cityId, 
